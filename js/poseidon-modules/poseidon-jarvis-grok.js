...
(function () {
  ...
  const TOOLS = [
    {
      ...
      parameters: {
        type: 'object',
        properties: {
          page_id: { type: 'string', enum: ['masterforecast','finance','recruitingdivision','processingcuk','j1division','ittech','contracts','j1housing','j1housingfinder','partneronboarding','j1contractanalysis','dashboard','tasks','calendar','videos','projects','partners','settings'], description: 'The page ID to open.' }
        },
        required: ['page_id']
      }
    },
    ...
    },
    {
      ...
      parameters: {
        ...
        properties: {
          page_id: {
            type: 'string',
            enum: ['masterforecast','finance','recruitingdivision','processingcuk','j1division','ittech','contracts','j1housing','j1housingfinder','partneronboarding','j1contractanalysis','dashboard','tasks','calendar','videos','projects','partners','settings'],
  ...
  function buildSystemInstructions() {
    return [
      ...
      'J1 DIVISION GROUP — the J1 System Dashboard sidebar groups three pages together under the J1 Division: (a) "J1 Division" (page id j1housing — housing & accommodations management plus the embedded J1 Housing Finder tab), (b) "Partner Onboarding" (page id partneronboarding — pipeline view of host companies moving through 5 stages: New Lead → MOU/NDA → Documentation → System Setup → Active, with a stalled-60-day flag and a standard onboarding checklist), and (c) "J1 Contract Analysis" (page id j1contractanalysis — side-by-side comparison of Alliance Abroad / CIEE / Green Heart sponsor contracts: fees, terms, insurance, response time, payment terms, plus fill-rate and pipeline-volume charts). For onboarding-pipeline questions go to partneronboarding; for sponsor-contract questions go to j1contractanalysis. Use go_to_page to navigate. The sidebar\'s remaining utility pages (Home / Tasks / Calendar / Tracker / Settings) are now grouped under "Other".',
      'CONTRACTS LIVE WHERE — the Sponsor Contracts page (page id contracts, Cruise Line Contract Negotiation Dashboard) lives ONLY on the Poseidon Master dashboard. It was removed from the J1 System Dashboard sidebar on 2026-04-27 because it duplicated the Poseidon copy. On the Poseidon contracts page you will also find a J1 Contract Analysis launcher (CTI Command Center) at the bottom — this was relocated there from the Poseidon J1 Division tab bar on the same date. If a user on the J1 dashboard asks about the Cruise Line Contract Negotiation or the J1 Contract Analysis launcher, redirect them to Poseidon Master via the cross-dashboard switcher pill.',
